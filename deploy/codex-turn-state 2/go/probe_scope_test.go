...
// --- probe scope does not invalidate templates ---------------------------

func TestProbeScopeChangeKeepsTemplates(t *testing.T) {
	// Editing the scope on the dashboard is the commonest reason configure runs
	// at all. Clearing there would throw away live templates every time the
	// operator ticked a box -- and a cleared cache looks exactly like "the probe
	// never ran".
	base := defaultConfig()
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			next := base
			tc.mutate(&next)
			if templatesInvalidatedBy(base, next) {
				t.Fatal("a probe-scope edit cleared the held templates")
			}
		})
	}

	// The control: a rule that really does invalidate templates still must.
	shorter := base
	shorter.TTLSeconds = base.TTLSeconds / 2
	if !templatesInvalidatedBy(base, shorter) {
		t.Fatal("a ttl_seconds change no longer clears templates; the control case is broken")
	}
}

// --- the firewall --------------------------------------------------------

func TestBusinessSubstitutesForAnAccountOutsideTheProbeScope(t *testing.T) {
	// §2.4: with probe_accounts and probe_proxies configured, the business path
	// must still substitute for an account that is NOT in the list. The lists
	// say which buckets the next probe run will try to fill; they say nothing
	// about which buckets may be used.
	dir := t.TempDir()
	now := wallClock()
	issued := now.Add(-5 * time.Minute)
	const (
		served = "codex-served.json" // holds a live bucket, absent from the scope
		scoped = "codex-scoped.json" // the only account in probe_accounts
		model  = "gpt-5.6-sol"
	)
	stored := storeRecordFor(served, model, issued, 292)
...
`, dir, scoped, testProxyWithPW)
	...
	resp := interceptAfter(t, request(served, model, degraded))
	got := outgoingHeader(resp)
	if got == "" {
		t.Fatal("probe_accounts filtered the business path: an account with a live bucket was not substituted")
	}
	if got != stored.Value {
		t.Fatalf("substituted a value that is not the stored template (len %d)", len(got))
	}
}
...
`, dir, scoped, testProxyWithPW)
	...
	resp := interceptAfter(t, request(scoped, model, fakeTokenSeed(312, now, 0x21)))
	if got := outgoingHeader(resp); got != "" {
		t.Fatalf("a request was rewritten with no bucket on disk (len %d)", len(got))
	}
	if after := snapshotDir(t, dir); !equalSnapshots(before, after) {
		t.Fatalf("the business path wrote to the store\nbefore: %v\nafter:  %v", before, after)
	}
}
...
func TestScopeSaveKeepsTemplates(t *testing.T) {
	dir := t.TempDir()
	now := wallClock()
	stored := storeRecordFor("codex-a.json", "gpt-5.6-sol", now.Add(-time.Minute), 292)
	mustWriteRecord(t, dir, stored)
	// Business role, not probe: the probe half never rewrites a request, so a
	// substitution is the only way to observe that the template survived.
...
`, dir))
	...
	// Prime the in-memory view by serving one request through the bucket.
	interceptAfter(t, request("codex-a.json", "gpt-5.6-sol", fakeTokenSeed(312, now, 0x11)))
	...
	// The template is still there and still used. Ticking a box on the dashboard
	// must not throw away a live card.
	out := interceptAfter(t, request("codex-a.json", "gpt-5.6-sol", fakeTokenSeed(312, now, 0x22)))
	if got := outgoingHeader(out); got != stored.Value {
		t.Fatalf("the template was lost when the scope was saved (got %d chars)", len(got))
	}
}
