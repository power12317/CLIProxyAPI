...
func TestKeylessClearRequiresConfirm(t *testing.T) {
	...
	mustWriteRecord(t, dir, storeRecord{
		AuthID: "codex-x.json", Model: "gpt-5.5", Len: 292,
		Value:    fakeToken(292, issued),
		IssuedAt: issued.UTC().Format(time.RFC3339), HarvestedAt: issued.UTC().Format(time.RFC3339),
	})
	...
	rel, err := bucketRelPath("codex-x.json", "gpt-5.5")
	if err != nil {
		t.Fatalf("bucketRelPath: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, rel)); err != nil {
		t.Errorf("bucket was removed despite the clear being refused for lack of confirm=1: %v", err)
	}
}
...
func TestKeylessClearOneBucket(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, rel)); !os.IsNotExist(err) {
		t.Errorf("bucket file still present after keyless clear (stat err: %v)", err)
	}
}

// Clearing everything via ?all=1 wipes the store.
func TestKeylessClearAll(t *testing.T) {
	...
	for _, m := range []string{"gpt-5.5", "gpt-5.6-sol"} {
		mustWriteRecord(t, dir, storeRecord{
			AuthID: "codex-x.json", Model: m, Len: 292,
			Value:    fakeToken(292, issued),
			IssuedAt: issued.UTC().Format(time.RFC3339), HarvestedAt: issued.UTC().Format(time.RFC3339),
		})
	}
	...
	if res.Cleared != 2 {
		t.Errorf("cleared %d buckets, want 2", res.Cleared)
	}
}
