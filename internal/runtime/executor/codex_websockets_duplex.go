...
func (e *CodexWebsocketsExecutor) streamCodexDuplex(
	...
	go func() {
		...
		processCreatePayload := func(payload []byte) bool {
			...
			authMu.Unlock()
			prepared, errPrepare := e.prepareCodexWebsocketStream(streamCtx, requestAuth, nextReq, nextOpts)
			if errPrepare != nil {
				fail(errPrepare)
				return false
			}
	...
	go func() {
		...
		for {
			...
			// Persist metadata as it arrives, before releasing it downstream. A
			// subsequent create on this socket must see the newly returned state.
			metadataMu.Lock()
			stateOwner := current
			metadataResponseID := gjson.GetBytes(payload, "response_id").String()
			if metadataResponseID == "" {
				metadataResponseID = gjson.GetBytes(payload, "response.id").String()
			}
			if metadataResponseID != "" && responseSettings[metadataResponseID] != nil {
				stateOwner = responseSettings[metadataResponseID]
			} else if !responseActive && len(pending) > 0 {
				stateOwner = pending[0]
			}
			metadataMu.Unlock()
			stateOwner.turnState.ObserveEvent(payload)
			authMu.Lock()
			...
			if eventType == "response.completed" || eventType == "response.done" || eventType == "response.incomplete" || eventType == "response.failed" || eventType == "error" {
				eventPrepared.turnState.LogResponse(ctx, e.cfg, true)
			}
