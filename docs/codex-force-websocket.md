...
The topic-owned WebSocket repair is integrated into `main`. Formal Git tags
use the repository's `vYYYY.MM.DD-N` scheme and trigger the standard release
and Docker workflows. The mainline image is published to
`ghcr.io/power12317/cliproxyapi` with the version tag and `latest`, including
Linux amd64 and arm64 images and their multi-architecture manifest.

Earlier `websocket-har-alignment-vYYYY.MM.DD-N` tags remain isolated historical
builds in `ghcr.io/power12317/cliproxyapi-websocket-har-alignment`. Their dedicated
workflow does not publish to the mainline package or update its `latest` tag.
